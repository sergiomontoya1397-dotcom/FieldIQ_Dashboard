...
nombre_lote = st.sidebar.text_input(
    "Nombre del lote",
    value="Lote 65–66"
)

unidad = st.sidebar.text_input(
    "Unidad",
    value="L/ha"
)
...
if uploaded_file:
    ...
    sub_pct = (
        get_pct("Clase 3: < -10%")
        +
        get_pct("Clase 2: -5% a -10%")
    )

    sobre_pct = (
        get_pct("Clase 4: +5% a +10%")
        +
        get_pct("Clase 5: > +10%")
    )
    ...
    tabla["Área (ha)"] = (
        tabla["Area_ha"].round(3)
    )

    tabla["% del total"] = (
        tabla["Porcentaje"].round(2)
    )
